class OpenedRubocopWorker
  ...
  def perform(payload)
    ...
    if initial_warnings.to_i < current_warnings.to_i
      ...
      client.create_status(full_name, merge_sha, :failure, context: 'RuboCop', description: "#{new_offenses} new offenses have been added. (#{current_warnings} total offenses)")
    else
      reduced_offenses = initial_warnings.to_i - current_warnings.to_i
      client.create_status(full_name, merge_sha, :success, context: 'RuboCop', description: "#{reduced_offenses} offenses have been removed. (#{current_warnings} total offenses)")
    end
  end
end